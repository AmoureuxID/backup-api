const ANIME_API_BASE = "https://www.sankavollerei.com/anime/samehadaku";
...
function randomId() {
  ...
  return `anime-bridge-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
...
async function fetchWithRetry(url, init, attempts = 2) {
  ...
  throw lastError ?? new Error("Anime bridge fetch failed");
}

function buildCacheControl(kind, path) {
  const normalized = String(path || "").toLowerCase();
  if (kind === "watch" || /(episode|stream)/i.test(normalized)) {
  ...
  if (kind === "detail" || /(detail|genre|list)/i.test(normalized)) {
...
function buildUpstreamUrl(path, query) {
  ...
  const url = new URL(`${ANIME_API_BASE}/${normalized}`);
...
export default async function handler(req, res) {
  ...
  res.setHeader("x-anime-bridge", "1");
  ...
  const configuredSecret = normalizePath(process.env.ANIME_BRIDGE_SECRET || "") ||
    normalizePath(process.env.PROXY_SECRET || "");
  if (!configuredSecret) {
    return res.status(500).json({ error: "Missing ANIME_BRIDGE_SECRET or PROXY_SECRET" });
  }
  ...
  try {
    ...
    return res.status(502).json({
      error: "Anime bridge transport failure",
